...
#  - REST API لبرنامج Ayman Auto Clicker
#  - إدارة كاملة: Pending / Active / Banned / Expired / Paused
#  - Ban / Unban / Renew / Pause / Unactivate / Reject
#  - Online Heartbeat (كل 120 ثانية) + Last Seen
#  - إعدادات SMTP لإرسال بريد عند التفعيل / التجديد
# ===============================================================
...
# مدة اعتبار الجهاز Online بعد آخر هارتبيت (بالثواني)
ONLINE_THRESHOLD_SECONDS = 180  # هارتبيت كل 120 ثانية + هامش

# Flask app
app = Flask(__name__)
app.secret_key = os.getenv("AYMAN_SERVER_SESSION_KEY", "CHANGE_ME_SESSION_SECRET_AYMAN")
...
def _default_settings():
    return {
        ...
        "smtp_user": "",
        "smtp_password": "",
        "smtp_sender": "Ayman Software <noreply@example.com>",
        "admin_notify_email": ""
    }
...
def load_db():
    ...
    if not os.path.exists(DB_FILE):
        return {"settings": defaults.copy(), "clients": []}
    ...
    # إذا كان الملف القديم عبارة عن list فقط
    if isinstance(data, list):
        return {"settings": defaults.copy(), "clients": data}
...
def send_email_smtp(to_email: str, subject: str, body: str, settings: dict) -> bool:
    """
    يرسل رسالة إيميل عبر SMTP حسب إعدادات settings.
    """
    ...
    smtp_sender = settings.get("smtp_sender") or smtp_user
    ...
    try:
        ...
        # إرسال نسخة إلى المطوّر إذا تم ضبط بريد
...
@app.route("/")
def home():
    return "<h2>Ayman Activation Server ✔</h2><p>Go to <a href='/admin'>Admin Panel</a></p>"
...
def api_request_activation():
    ...
    if client:
        ...
        if client.get("status") == "banned":
            return jsonify({
                "status": "banned",
                "message": client.get("banned_reason", "هذا الجهاز محظور من قبل النظام.")
            }), 403

        current_status = client.get("status")

        if current_status == "active":
            # لا نعدل بيانات الهوية بعد التفعيل
            changed = False
            ...
            # مسموح بتعديل البيانات في الحالات الأخرى
            changed = False
...
# ============================================================
#  3) API: Online Heartbeat
# ============================================================

@app.route("/api/online_ping", methods=["GET", "POST"])
def api_online_ping():
    """
    يستدعيه البرنامج كل 120 ثانية تقريباً ليحدّث حالة Online.
    يمكن الإرسال عبر GET أو POST:
      GET /api/online_ping?machine_id=XXXX
      أو JSON: {"machine_id": "..."}
    """
    db = load_db()
    clients = db["clients"]

    raw_mid = request.args.get("machine_id", "")
    if not raw_mid and request.is_json:
        data = request.get_json(silent=True) or {}
        raw_mid = data.get("machine_id", "")

    raw_mid = (raw_mid or "").strip()
    if not raw_mid:
        return jsonify({"status": "error", "message": "machine_id مفقود"}), 400
    ...
    now = now_iso()
    client["last_online_at"] = now
    client["online"] = True
    client["updated_at"] = now
    save_db(db)

    return jsonify({"status": "ok", "last_online_at": now})
...
<head>
  ...
  <style>
    ...
    .online-dot  { width:10px; height:10px; border-radius:50%; display:inline-block; margin-left:4px; }
    .online-true  { background:#4caf50; }
    .online-false { background:#9e9e9e; }
  </style>
</head>
<body>
        ...
        <table class="table table-sm table-hover align-middle">
          ...
          {% for c in active %}
            <tr class="status-active">
              ...
              <td>
                {% if c.online_flag %}
                  <span class="online-dot online-true"></span>Online
                {% else %}
                  <span class="online-dot online-false"></span>Offline
                {% endif %}
              </td>
...
def admin_dashboard():
    ...
    # تجهيز معلومات online / last_seen
    for c in clients:
        ts_str = c.get("last_online_at")
        online_flag = False
        last_seen = "-"
        if ts_str:
            try:
                ...
                online_flag = diff <= ONLINE_THRESHOLD_SECONDS
                last_seen = ts_str.replace("T", " ")[:19]
            except Exception:
                online_flag = False
        c["online_flag"] = online_flag
  ...
  {% if client %}
    <div class="card mb-3">
      ...
      <div class="card-body">
        ...
        <p><strong>آخر ظهور Online:</strong> {{ client.last_seen or "-" }}</p>
      </div>
    </div>
    <div class="card">
      <div class="card-header">
...
def admin_device(mid):
    ...
    if client:
        now = now_utc()
        ts_str = client.get("last_online_at")
        last_seen = "-"
        if ts_str:
            try:
                dt = datetime.fromisoformat(ts_str)
...
# ============================================================
#  7) صفحة إعدادات اللوحة (تشمل SMTP)
# ============================================================
...
<div class="container my-4">
  ...
  <form method="post" class="card shadow p-3">
    ...
    <div class="mb-3">
      <label class="form-label">رقم الواتساب الذي يظهر للعميل</label>
    ...
    <div class="form-check form-switch mb-3">
      ...
      <label class="form-check-label">تفعيل إرسال الإيميل للعميل</label>
    </div>
    ...
    <div class="mb-3">
      <label class="form-label">SMTP User (البريد المرسل)</label>
    ...
    <div class="mb-3">
      <label class="form-label">From (اسم المرسل الظاهر)</label>
    ...
    <div class="form-check form-switch mb-3">
      ...
      <label class="form-check-label">استخدام SMTP_SSL (إن لم يفعّل سيتم استخدام STARTTLS)</label>
    </div>
...
def admin_settings():
    ...
    if request.method == "POST":
        ...
        settings["smtp_ssl"] = bool(request.form.get("smtp_ssl"))
...
def test_smtp():
    ...
    if not test_email:
        flash("لا يوجد بريد لاختبار الإرسال. ضع بريد مطوّر أو SMTP User.", "danger")
        return redirect(url_for("admin_settings"))
...
# ============================================================
#  8) إجراءات الأدمن (تفعيل، تجديد، حظر، إلغاء حظر، إلخ)
# ============================================================
